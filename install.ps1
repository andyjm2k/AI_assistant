...
Write-Host "`n[2/9] Initializing mcp-browser-use..." -ForegroundColor Yellow
...
Write-Host "`n[3/9] Creating venv and installing Python dependencies..." -ForegroundColor Yellow
...
Write-Host "`n[4/9] Installing Playwright browsers..." -ForegroundColor Yellow
& $venvPython -m playwright install
if ($LASTEXITCODE -ne 0) { Write-Host "Playwright install failed." -ForegroundColor Red; exit 1 }

# 5. Node dependencies
Write-Host "`n[5/9] Installing Node.js dependencies..." -ForegroundColor Yellow
& npm install
if ($LASTEXITCODE -ne 0) { Write-Host "npm install failed." -ForegroundColor Red; exit 1 }

# 6. mcp-browser-use: uv sync and playwright
Write-Host "`n[6/9] Setting up mcp-browser-use (uv sync + playwright)..." -ForegroundColor Yellow
...
Write-Host "`n[7/9] Creating .env and required directories..." -ForegroundColor Yellow
...
Write-Host "`n[8/9] Configuration wizard (API keys, Telegram, etc.)..." -ForegroundColor Yellow
& $venvPython scripts/install_wizard.py
if ($LASTEXITCODE -ne 0) { Write-Host "Wizard failed." -ForegroundColor Red; exit 1 }

# 9. Verification
Write-Host "`n[9/9] Verifying installation..." -ForegroundColor Yellow
