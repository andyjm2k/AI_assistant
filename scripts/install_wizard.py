...
def _get_key_from_env_content(content: str, key: str) -> str:
    """Return KEY's value from .env-style content, or empty string when unset."""
...
def _use_existing_secret(value: str, *disallowed: str) -> bool:
    """Return True when an existing env value is non-empty and not one of the placeholders."""
    normalized = value.strip()
    return bool(normalized and normalized not in set(disallowed))
...
def run_wizard(project_root: Path, env_path: Path) -> bool:
    ...
    shared_agent_secret = (
        _get_key_from_env_content(content, "MCP_BROWSER_SERVER_SECRET")
        or _get_key_from_env_content(content, "CATBOT_AGENT_SECRET")
        or _get_key_from_env_content(content, "AUTOGEN_TEAM_SECRET")
        or secrets.token_urlsafe(48)
    )
    existing_jwt_secret = _get_key_from_env_content(content, "JWT_SECRET")
    jwt_secret = (
        existing_jwt_secret
        if _use_existing_secret(existing_jwt_secret, "change-this-secret-in-production")
    ...
    print("   Shared agent secret:", "configured")
    print("   For HTTPS from other devices, run: mkcert", https_hostname, "localhost 127.0.0.1 <your-ip>")
    print("   AutoGen endpoint auth is enabled by default; Docker code execution stays disabled until you opt in.")
