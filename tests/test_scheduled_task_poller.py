...
def test_select_due_task_ids_reads_all_valid_due_items_in_order():
    payload = {
        "taskItems": [
            {"taskId": "bad"},
            {"taskId": 3, "isDue": True},
    ...
    assert select_due_task_ids(payload) == [3, 4]
...
def test_poller_poll_once_attempts_all_due_tasks_without_blocking(monkeypatch):
    tmp_dir = _make_test_dir()
    users_file = tmp_dir / "auth_users.json"
    users_file.write_text(json.dumps({"alice": {"salt": "x", "password_hash": "y"}}), encoding="utf-8")
    ...
    def fake_request(*, method, url, bearer_token, timeout_seconds, body=None, skip_tls_verify=False):
        ...
        if method == "GET" and url.endswith("/v1/todo/due"):
            return 200, {"taskItems": [{"taskId": 2}, {"taskId": 3}]}, ""
        if method == "POST" and url.endswith("/v1/todo/execute"):
            if body == {"taskId": 2}:
                return 409, {"detail": "Task 2 is already executing."}, ""
            if body == {"taskId": 3}:
                return 200, {"status": "executing"}, ""
        return 500, {"detail": "unexpected"}, "unexpected"
    ...
    try:
        poller.poll_once()
        post_bodies = [c["body"] for c in calls if c["method"] == "POST"]
        assert post_bodies == [{"taskId": 2}, {"taskId": 3}]
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
